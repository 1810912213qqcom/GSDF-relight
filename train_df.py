...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, unfreeze_iterations, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # Alpha Regularization Loss to remove background floaters
        # Penalize accumulated opacity where GT is dark
        if 'alpha' in render_pkg:
            acc_alpha = render_pkg['alpha']
            gt_intensity = gt_image.mean(dim=0, keepdim=True)
            # Weight: 1.0 where GT is black, 0.0 where GT is white
            alpha_weight = torch.clamp(1.0 - gt_intensity, 0.0, 1.0)
            loss_alpha = (acc_alpha * alpha_weight).mean()
            loss += 0.05 * loss_alpha # Weight can be tuned
